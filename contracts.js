...
CREDENTIALS_HUB_ABI = [
    ...
    "function addDegreeCredential(uint256 tokenId, bytes32 metadataHash, uint64 issueDate, uint64 expiryDate, uint16 gpa, uint8 degreeCategory) external returns (uint256)",
    "function issueDegreeCredential(uint256 tokenId, bytes32 metadataHash, uint64 issueDate, uint64 expiryDate, uint16 gpa, uint8 degreeCategory) external returns (uint256)",
    "function addCertificationCredential(uint256 tokenId, bytes32 metadataHash, uint64 issueDate, uint64 expiryDate, uint8 domain) external returns (uint256)",
    "function issueCertificationCredential(uint256 tokenId, bytes32 metadataHash, uint64 issueDate, uint64 expiryDate, uint8 domain) external returns (uint256)",
    "function getTotalWorkExperience(uint256 tokenId) external view returns (uint256 totalYears, uint256 remainingMonths)",
    "function degreeGPA(uint256 credentialId) external view returns (uint16)",
    ...
    "event DegreeCredentialAdded(uint256 indexed tokenId, uint256 indexed credentialId, uint16 gpa, uint8 degreeCategory)",
    "event CertificationCredentialAdded(uint256 indexed tokenId, uint256 indexed credentialId, uint8 domain)"
];
...
SOCIAL_HUB_ABI = [
    ...
    "function endorseStandardSkill(uint256 subjectTokenId, uint256 endorserTokenId, uint8 skill, string calldata comment) external",
    "function getStandardSkillEndorsementCount(uint256 tokenId, uint8 skill) external view returns (uint256)",
    "function getAllStandardSkillCounts(uint256 tokenId) external view returns (uint256[16] memory counts)",
    "function getSkillCountsByCategory(uint256 tokenId, uint8 category) external view returns (uint8[] memory skillIds, uint256[] memory counts)",
    "function getSkillCategory(uint8 skill) external pure returns (uint8)",
    "function hasEndorsedStandardSkill(uint256 subjectTokenId, uint8 skill, uint256 endorserTokenId) external view returns (bool)",
    "function getEndorsementCount(uint256 tokenId, bytes32 skillHash) external view returns (uint256)",
    "function getSkillEndorsements(uint256 tokenId, bytes32 skillHash) external view returns (tuple(uint256 endorserId, bytes32 skillHash, uint64 endorsedAt, string comment)[] memory)",
...
CONFIG = {
    ...
    CONTRACTS: {
        ...
        CREDENTIALS_HUB: '0x180B2851dCF33f6F29cE5B79141b0F3Aa0Ed22Ed',
