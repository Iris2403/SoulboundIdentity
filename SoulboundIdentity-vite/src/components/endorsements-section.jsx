...
export function EndorsementsSection({ endorsements, endorsementCounts, loading, contracts, selectedToken, userTokens, showNotification, onReload }) {
    const [showEndorseModal, setShowEndorseModal] = useState(false);
    const [endorseData, setEndorseData] = useState({ targetTokenId: '', endorserTokenId: '', skillName: '', comment: '' });

    const endorsementsBySkill = {};
    if (endorsements && endorsements.length > 0) {
        endorsements.forEach(end => {
            const skillHash = end.skillHash;
            if (!endorsementsBySkill[skillHash]) endorsementsBySkill[skillHash] = [];
    ...
    const handleEndorseSkill = async () => {
        try {
            ...
            if (targetId === endorserId) { showNotification('You cannot endorse yourself!', 'error'); return; }
            ...
            try {
                ...
                if (owner.toLowerCase() !== myAddress.toLowerCase()) { showNotification(`You don't own Token #${endorserId}!`, 'error'); return; }
        ...
        <>
            <Card>
                <div className="section-header">
                    ...
                    <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                        <span className="badge" style={{ background: 'rgba(102, 126, 234, 0.8)' }}>{Object.keys(endorsementsBySkill).length} Skills</span>
                        <span className="badge">{endorsements?.length || 0} Total</span>
                        <Button onClick={() => setShowEndorseModal(true)} style={{ padding: '8px 16px', fontSize: '13px' }}>+ Endorse Skill</Button>
                    </div>
                </div>
                {loading ? <LoadingSpinner /> : !endorsements || endorsements.length === 0 ? (
                                        ...
                                        {skillEndorsements.map((endorsement, idx) => (
                                            <div key={idx} className="endorsement-item">
                                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
                                                    <div style={{ fontWeight: '500', color: 'var(--beige)' }}>From Token #{endorsement.endorserId.toString()}</div>
            ...
            <Modal isOpen={showEndorseModal} onClose={() => setShowEndorseModal(false)} title="Endorse Skill">
                <div className="endorse-form">
                    <Input label="Token ID to Endorse" value={endorseData.targetTokenId} onChange={(val) => setEndorseData({ ...endorseData, targetTokenId: val })} placeholder="Enter token ID" type="number" required />
                    <Select label="Your Token ID (Endorser)" value={endorseData.endorserTokenId} onChange={(val) => setEndorseData({ ...endorseData, endorserTokenId: val })} options={userTokens.map(t => ({ value: t.id.toString(), label: `Token #${t.id}` }))} required />
                    <Input label="Skill Name" value={endorseData.skillName} onChange={(val) => setEndorseData({ ...endorseData, skillName: val })} placeholder="e.g., Solidity, Project Management, Python" required />
                    <div>
                        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px' }}>
                            <label className="input-label">Comment (Optional)</label>
                            <span style={{ fontSize: '0.8rem', color: commentLength > commentLimit ? 'var(--warning)' : 'var(--text-muted)' }}>{commentLength}/{commentLimit}{commentLength > commentLimit && ' ⚠️'}</span>
                        </div>
                        <TextArea value={endorseData.comment} onChange={(val) => setEndorseData({ ...endorseData, comment: val })} placeholder="Add a comment about this skill..." rows={3} />
                    ...
                    <div className="modal-actions">
                        ...
                        <Button onClick={handleEndorseSkill}>Endorse Skill</Button>
                    </div>
                </div>
            </Modal>
        </>
