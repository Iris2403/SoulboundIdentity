...
export function AnalyticsTab({ contracts, selectedToken, showNotification }) {
    ...
    if (!stats) return <Card><div className="empty-message">Failed to load analytics</div></Card>;
        ...
        <div className="analytics-container">
            ...
            <div className="analytics-dashboard" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '24px' }}>
                <Card className="stat-card" style={{ padding: '24px' }}>
                    ...
                    <div className="big-number" style={{ fontSize: '3rem', fontWeight: 'bold', color: 'var(--teal-light, #06b6d4)', marginBottom: '8px' }}>{stats.credentials.total}</div>
                    ...
                    <div className="stat-breakdown" style={{ display: 'flex', flexDirection: 'column', gap: '8px', paddingTop: '16px', borderTop: '1px solid var(--border-color, rgba(168, 178, 193, 0.1))' }}>
                        <div className="breakdown-item"><span>🎓 Degrees: {stats.credentials.degrees}</span></div>
                        <div className="breakdown-item"><span>📋 Certifications: {stats.credentials.certifications}</span></div>
                        <div className="breakdown-item"><span>💼 Work Experience: {stats.credentials.workExperience}</span></div>
                        <div className="breakdown-item"><span>⚡ Skills: {stats.credentials.skills}</span></div>
                    </div>
                </Card>
                ...
                <Card className="stat-card" style={{ padding: '24px' }}>
                    ...
                    <div className="big-number" style={{ fontSize: '3rem', fontWeight: 'bold', color: 'var(--teal-light, #06b6d4)', marginBottom: '8px' }}>
                        {stats.reputation.average}<span style={{ fontSize: '1.5rem', color: 'var(--gray, #6b7589)' }}>/100</span>
                    </div>
                    <div className="stat-label" style={{ color: 'var(--gray, #6b7589)', marginBottom: '16px' }}>Average Score</div>
                    <div className="stat-breakdown" style={{ display: 'flex', flexDirection: 'column', gap: '8px', paddingTop: '16px', borderTop: '1px solid var(--border-color, rgba(168, 178, 193, 0.1))' }}>
                        <div className="breakdown-item"><span>Total Reviews: {stats.reputation.total}</span></div>
                        <div className="breakdown-item"><span>✓ Verified: {stats.reputation.verified}</span></div>
                        <div className="breakdown-item"><span>Verification Rate: {stats.reputation.total > 0 ? Math.round((stats.reputation.verified / stats.reputation.total) * 100) : 0}%</span></div>
                    </div>
                </Card>
                ...
                <Card className="stat-card" style={{ padding: '24px' }}>
                    ...
                    <div className="stat-breakdown" style={{ display: 'flex', flexDirection: 'column', gap: '12px', paddingTop: '16px' }}>
                        <div className="breakdown-item" style={{ fontSize: '1.1rem' }}><span>👍 Endorsements: <strong>{stats.social.endorsements}</strong></span></div>
                        <div className="breakdown-item" style={{ fontSize: '1.1rem' }}><span>🚀 Total Projects: <strong>{stats.social.projects}</strong></span></div>
                        <div className="breakdown-item" style={{ fontSize: '1.1rem' }}><span>✅ Completed: <strong>{stats.social.completedProjects}</strong></span></div>
                        <div className="breakdown-item" style={{ fontSize: '1.1rem' }}><span>⚡ Active: <strong>{stats.social.activeProjects}</strong></span></div>
                    </div>
                </Card>
            </div>
        </div>
