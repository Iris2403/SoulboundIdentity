        ...
        <div className="reputation-section">
            ...
            <Card>
                <div className="section-header">
                    <h3>ZK Reputation Proof</h3>
                </div>
                <p style={{ color: 'var(--text-secondary)', marginBottom: '20px', fontSize: '0.95rem' }}>
                    Prove your score is above a threshold without revealing the actual value.
                </p>
                {reputation && (
                    <div style={{ marginBottom: '20px', padding: '12px', background: 'rgba(6, 182, 212, 0.05)', borderRadius: '8px', borderLeft: '3px solid var(--teal)' }}>
                        <span style={{ color: 'var(--text-muted)', fontSize: '0.85rem' }}>Your current score: </span>
                        <span style={{ color: 'var(--teal-light)', fontWeight: '700', fontSize: '1.1rem' }}>{reputation.averageScore.toString()}</span>
                    </div>
                )}
                <Input
                ...
                <div style={{ marginTop: '16px' }}>
                    ...
                    >
                        {zkpStatus === 'generating' ? 'Generating Proof...' :
                         zkpStatus === 'verifying' ? 'Verifying On-Chain...' :
                         'Generate ZK Proof'}
                    </Button>
                </div>
                {zkpMessage && (
                    <div style={{
                        ...
                        padding: '12px',
                        borderRadius: '8px',
                        background: zkpStatus === 'success' ? 'rgba(16, 185, 129, 0.1)' :
                                    zkpStatus === 'error' ? 'rgba(239, 68, 68, 0.1)' :
                                    'rgba(6, 182, 212, 0.05)',
                        border: `1px solid ${zkpStatus === 'success' ? 'rgba(16, 185, 129, 0.3)' :
                                              zkpStatus === 'error' ? 'rgba(239, 68, 68, 0.3)' :
                                              'rgba(6, 182, 212, 0.2)'}`,
                        color: zkpStatus === 'success' ? 'var(--success)' :
                               zkpStatus === 'error' ? 'var(--error)' :
                               'var(--text-secondary)',
                        fontSize: '0.9rem'
                    }}>
                        {(zkpStatus === 'generating' || zkpStatus === 'verifying') ? '⏳ ' :
                          zkpStatus === 'success' ? '✓ ' : '✗ '}
                        {zkpMessage}
                    </div>
                )}
